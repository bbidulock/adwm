...
#if defined IMLIB2
void
imlib2_removebutton(ButtonImage *bi)
{
	if (bi->pixmap.image) {
...
imlib2_drawnormal(AScreen *ds, Client *c)
{
	...
	if (ds->dc.w < textw(ds, c->name, (c == sel) ? Selected : (c->is.focused ? Focused : Normal))) {
		...
		imlib2_drawbutton(ds, c, CloseBtn,
			   c == sel ? ds->style.color.sele : ds->style.color.norm,
			   ds->dc.w);
		goto end;
	}
...
static Bool
imlib2_initext(char *path, ButtonImage *bi)
{
	Imlib_Image image;
...
Bool
imlib2_initpng(char *path, ButtonImage *bi)
{
	return imlib2_initext(path, bi);
}

Bool
imlib2_initsvg(char *path, ButtonImage *bi)
{
	return imlib2_initext(path, bi);
}

Bool
imlib2_initxpm(char *path, ButtonImage *bi)
{
...
Bool
imlib2_initxbm(char *path, ButtonImage *bi)
{
